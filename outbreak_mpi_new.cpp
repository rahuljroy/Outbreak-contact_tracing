...
void get_infected(char* filename, long int no_infected, long int **infected, int rank, int *infected_or_not, int *quarantined){
    long int u, i=0;
    string data;
...
int main(int argc, char** argv){
    ...
    long int *new_infected, **infected, *new_quarantined;
    long int *old_arraysize, *new_arraysize;
    old_arraysize = new long int[num_procs];
    new_arraysize = new long int[num_procs];
    long int initial_infected=0, current_infected=0, neighbour_size, size=0, new_size=0, totalsize=0, new_infected_size=0, current_quarantined=0, new_quarantine_size=0;
    

    vector<long int>neighbours;
    infected = new long int*[num_procs];
    new_infected = new long int[100];
    new_quarantined = new long int[100];
    ...
    get_infected(file, initial_infected, infected, myrank, infected_or_not, quarantined);
    ...
    while(totalsize){
                    ...
                    for (k=0; k<neighbours.size(); k++){
                        ...
                        if (quarantined[neighbours[k]]==0){
                            if (num < 0.5){
                                ...
                                if (current_quarantined<new_quarantine_size){
        ...
        for (l=0; l<num_procs; l++){
            ...
            // if (myrank==0)
            cout<<myrank<<" : "<<new_arraysize[l]<<endl;
        }
