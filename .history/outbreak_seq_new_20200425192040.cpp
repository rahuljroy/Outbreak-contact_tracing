...
void get_neighbours(vector< vector<long int> > graph, long int u, vector< vector<long int> >& neighbours, vector<int>& quarantined, vector<int> infected_flag, vector<int> quarantined_flag){

    int i=0, j=0, flag = 0;

    for (i=0; i<graph[u].size(); i++){
        if (infected_flag[graph[u][i]!=1])
    }
}
...
int main(){
    ...
    vector<int> infected_or_not, prev_infected, processed, quarantined;
    set<int> indices;
    ...
    while (flag!=0){
        ...
        for (j=0; j<infected[i].size(); j++){
            ...
            infected_flag.resize(no_vertices, 0);
            quarantined_flag.resize(no_vertices, 0);
            for (k=i; k<10; k++){
                get_neighbours(day_graphs[0].graph_for_the_day, infected[i][j], neighbours, quarantined, infected_flag, quarantined_flag);
                // cout<<k<<endl;
            }
