...
int main(int argc, char** argv){
    ...
    vector<int> infected_or_not, prev_infected;
    set<int> indices;
    ...
    prev_infected.resize(no_vertices);
    ...
    while (flag!=0){
        for (j=0; j<infected[i].size(); j++){
            if (infected_or_not[infected[i][j]]!=1){
                ...
                // }
                cout<<"reached 1"<<endl;
                while (indices.size() < (int) (node_weights[infected[i][j]][0]*neighbours[infected[i][j]].size()/10)){
