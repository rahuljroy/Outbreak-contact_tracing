...
int main(int argc, char** argv){
    ...
    while(totalsize){
        ...
        for(l=0;l<num_procs;l++){
            for(k=0; k<new_arraysize[l];k++){
                ...
                infected_day[infected[j][k]] = day+2;
                quarantined_day[infected_day[l][k]] = day+2;
            }
        }
    ...
    if(myrank==0){
        ...
        printf("\nTime taken for parallel code for outbreak, inckuding graph time,  is, %e", (t2-t1+graph_end-graph_start));
    }
    
    long int infected_count=0, qurantined_count=0, uninfected_count=0
