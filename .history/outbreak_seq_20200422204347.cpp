...
int main(int argc, char** argv){
    ...
    // for (i=0; i<infected.size(); i++){
    for (j=0; j<infected[0].size(); j++){
        neighbours[infected[0][j]] = get_neighbours(day_graphs[0].graph_for_the_day, infected[i][j]);
        for (k=0; k<neighbours[infected[i][j]].size(); k++){
            cout<<"neighbour of the infected guy "<<infected[i][j]<<" is: "<<neighbours[infected[i][j]][k]<<endl;
        }
    }
    // }
}