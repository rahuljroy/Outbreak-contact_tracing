...
long int get_no_lines(string filename){
...
void build_graph(string filename, long int no_vertices, vector<vector<long int>>& graph){
...
int main(int argc, char** argv){
    ...
    for (i=0; i<no_vertices; i++){
        for (j=0; j<day_graphs[6].graph_for_the_day.size(); j++){
            cout<<"the node weight of "<<i<<" is "<<day_graphs[6].graph_for_the_day[j];
        }
    }
}