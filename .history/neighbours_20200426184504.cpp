...
int main(){
    ...
    long int u, v, graph_number, infected_count=0, quarantined_count = 0;
    double num;
    ...
    cout<<"the number of infected people on day 0 is "<<infected.size()<<endl;
    ...
    while(count!=0){
        count = infected.size();
        for (j=0; j<count; j++){
            // get_neighbours(day_graphs.graph_for_the_day, infected[j], neighbours, quarantined, processed);
            ...
            for(k=0; k<neighbours[infected[j]].size(); k++){
                ...
                if(quarantined[neighbours[infected[j]][k]]==0){
                    if (num<0.5){
        ...
        cout<<"new size of infected array in day "<<i+2<<" is "<<current_infected.size()<<endl;
        i++;
